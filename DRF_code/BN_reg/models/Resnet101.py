...
class Resnet101(nn.Module):
    def __init__(self, depth=None, dataset='data.cifar10', cfg=None):
        ...
        # 每层 两个 残差块
        if depth == 101:
            self.layer1 = nn.Sequential(BaseRestBlockDownsample(cfg[0:4], [1, 1]),  # 残差块 图像大小 不变
                                        ResNetBasicBlock(cfg[3:7], 1),
                                        ResNetBasicBlock(cfg[6:10], 1))  # (64, 64, (3, 3)) * 2

            self.layer2 = nn.Sequential(BaseRestBlockDownsample(cfg[9:13], [2, 1]),  # 图像大小 减倍
                                        ResNetBasicBlock(cfg[12:16], 1),
                                        ResNetBasicBlock(cfg[15:19], 1),
                                        ResNetBasicBlock(cfg[18:22], 1))

            self.layer3 = nn.Sequential(BaseRestBlockDownsample(cfg[21:25], [2, 1]),  # 通道加倍，图像大小减半
            ...
            self.layer4 = nn.Sequential(BaseRestBlockDownsample(cfg[90:94], [2, 1]),  # 通道加倍，图像大小减半
        ...
        if dataset == 'data.cifar10':
            ...
            self.a = 100
        self.fc = nn.Linear(cfg[-1], self.a)  # 平均池化（05flex，05flex）可以确定 输入个数
