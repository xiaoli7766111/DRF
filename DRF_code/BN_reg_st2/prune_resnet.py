...
parser.add_argument('--model', default='NNews/R18_ratio/ssl0002/Resnet_cifar10_100.model', type=str,
                    help='path to the model (default: none)')

parser.add_argument('--save', default='prune/r18/ssl0002', type=str,
        ...
        # conv 模块
            ...
            if conv_count == 0:
                ...
                if args.depth == 18:
                    ...
                    elif conv_count % 3 == 2 and conv_count != 0:
                        idx0 = np.squeeze(np.argwhere(np.asarray(start_mask.cpu().detach().numpy())))
                        idx1 = np.squeeze(np.argwhere(np.asarray(end_mask.cpu().detach().numpy())))
                        numm = idx1
